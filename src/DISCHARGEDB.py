...
class DISCHARGEDB:
    ...
    def xlxsTosql(self, database='dischargedb3', ip='localhost', port='3306', un='root', pw='123', fip_xlsx='H:/cloud/cloud_data/Projects/DISCHARGEDB/data/tables/xlsx/discharge_master_01092020.xlsx'):
        ...
        #         mysql_path = 'mysql://' + un + ':' + pw + '@' + ip + '/?charset=utf8'
        #         engine = create_engine(mysql_path, encoding="utf-8", echo=False)
        #         with engine.connect() as con:
        #             command_use = 'use ' + database + ';'
        #             con.execute(command_use)
        #         _, filename, _ = splitFilePath(fip_xlsx)
        #         table = filename + '_' + sheet_name
        ...
        sheet_names = xls.sheet_names()
        for sheet_name in sheet_names:
            #df = pd.read_excel(fip_xlsx, sheet_name=sheet_name, index_col=0)
            df = pd.read_excel(fip_xlsx, sheet_name=sheet_name)
...
# Load settings
filepath_settings = 'H:/cloud/cloud_data/Projects/DISCHARGEDB/data/settings.json'
...
path = 'H:/cloud/cloud_data/Projects/DISCHARGEDB/src/scripts/set_primary_key.sql'
file = open(path)
#engine = create_engine(db_url)
escaped_sql = sqlalchemy.text(file.read())
escaped_sql.text = escaped_sql.text.replace('TABLE_VAR', 'v_a05_ses_en_b')
c = engine.execute(escaped_sql)
...
sas2db --db mysql+mysqlconnector://root@localhost:3306/dischargedb3 H:/cloud/cloud_data/Projects/DISCHARGEDB/data/tmp/ecrf/v_g02_ct_reading_a.sas7bdat
